...
## 1) Add your keys
- Place any **OpenSSH** public keys for human access under `files/ssh/` matching `monadmin*.pub`
  (You can create one from a PEM private key with: `ssh-keygen -y -f monadmin.pem > files/ssh/monadmin-from-pem.pub`)
- OR paste key strings into `group_vars/all.yml` under `monadmin_keys_human`.
- Optional automation key: `monadmin_automation_pubkey` and restrict with `ansible_controller_cidrs`.
...
ssh -i ~/.ssh/yourkey monadmin@YOUR_PI

# B) Enforce key-only auth
#    Set monadmin_disable_password_auth: true in group_vars/all.yml, then:
ansible-playbook -i inventories/prod/inventory.ini playbooks/users.yml
